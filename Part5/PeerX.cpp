...
transferCommon(std::shared_ptr<StateMgr> mySM, bool reportInfoParam)
{
	...
	while(mySM->isRunning()) {
		...
		if (now >= absoluteTimeout) {
			tv.tv_sec = 0; // Re-setting the Timeout back to 0s since it exceeded absoluteTimeout
			tv.tv_usec = 0;
			mySM->postEvent(TM);
		} else {
			...
			tv.tv_usec = absoluteTimeout - now; // Setting the Timeout to time remaining before absolute Timeout
			...
			if (rv == 0) {
				...
				if( FD_ISSET(consoleInId, &set ) ) {
				    ...
				    myBytesRead = PE(myRead(consoleInId, &byteToReceive, LINEMAX_SAFE));

				    //str_rcv = byteToReceive[0] + byteToReceive[1] + "\n";

				    //Loop to check if "&c\n" is detected
                    for ( int i = 0; i < myBytesRead-2; i++)
                    {

					if (byteToReceive[i] == CANC_C[0] && byteToReceive[i+1] == CANC_C[1] && byteToReceive[i+2] == CANC_C[2])
				    {
						KbCan = true;
				       mySM->postEvent(KB_C);
