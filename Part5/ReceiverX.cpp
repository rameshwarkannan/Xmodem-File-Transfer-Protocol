...
void ReceiverX::clearCan()
{
	...
	//Run until the condition mentioned above is satisfied
	while(cond){

	int myBytesRead = PE(myReadcond( mediumD, &character, 1, 1, dSECS_PER_UNIT*TM_2CHAR, dSECS_PER_UNIT*TM_2CHAR));

	//Check if something other than CAN or bytes read is 0 over a 2CHAR timeout period
	if (character != CAN || myBytesRead ==0 ){
...
{
	// ********** you will need to fill in this function ***********---------done
	char character;
    bool cond = true;
		int myBytesRead = 0;
		...
		int myBytesRead = PE(myReadcond( mediumD, &character, 1, 1, dSECS_PER_UNIT*TM_CHAR, dSECS_PER_UNIT*TM_CHAR));

		//Check if the bytes read is 0 over a 1-second period
		if (myBytesRead ==0 ){
